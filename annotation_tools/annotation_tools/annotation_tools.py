...
app.config['MONGO_URI'] = 'mongodb://'+cfg.MONGO_HOST + \
    ':'+str(cfg.MONGO_PORT)+'/'+cfg.MONGO_DBNAME
...
def edit_task():
    ...
    if 'image_ids' in request.args:
        ...
        if 'category_id' in request.args:
            ...
            annos = mongo.db.annotation.find({"category_id": category_id}, projection={
                                             'image_id': True, '_id': False})  # .sort([('image_id', 1)])
        ...
        # Else just grab all of the images.
            ...
            images = mongo.db.image.find(
                projection={'id': True, '_id': False}).sort([('id', 1)])
    ...
    for annotation in annotations:
        ...
        if '_id' in annotation:
            if 'deleted' in annotation and annotation['deleted']:
                ...
                result = mongo.db.annotation.replace_one(
                    {'_id': annotation['_id']}, annotation)
        else:
            if 'deleted' in annotation and annotation['deleted']:
                ...
                mongo.db.annotation.replace_one(
                    {'id': annotation['id']}, annotation, upsert=True)
...
def bbox_task(task_id):
    ...
    for image_id in bbox_task['image_ids']:
        image = mongo.db.image.find_one_or_404(
            {'id': image_id}, projection={'_id': False})
        tasks.append({
            'image': image,
            'annotations': []
        })
    ...
    categories = [mongo.db.category.find_one_or_404(
    ...
    task_instructions = mongo.db.bbox_task_instructions.find_one_or_404(
        {'id': task_instructions_id}, projection={'_id': False})
...
def bbox_task_save():
    ...
    insert_res = mongo.db.bbox_task_result.insert_one(
        task_result, bypass_document_validation=True)
